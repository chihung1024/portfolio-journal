# =========================================================================================
# == Python 週末完整校驗腳本 (v3.7 - Multi-Market Aware Resilient)
# =========================================================================================
import os
...
import time
import pandas as pd
import pandas_market_calendars as mcal # 【依賴】導入市場日曆函式庫
...
def robust_request(func, max_retries=3, delay=5, name="Request"):
    ...
    for attempt in range(1, max_retries + 1):
        try:
            ...
            if "YFPricesMissingError" in str(type(e)):
                 print(f"資訊: {name} 請求失敗，因為該日期區間無數據: {e}")
                 return None
...
# ========================= 【核心強化 D - 開始】 =========================
# == 新增：市場感知的日曆選擇器，為不同市場提供正確的交易日曆
# =========================================================================================
def get_market_calendar(symbol: str):
    """
    根據金融商品代碼的後綴，回傳對應市場的交易日曆物件。
    :param symbol: 金融商品代碼。
    :return: 一個 pandas_market_calendars 的實例。
    """
    symbol_upper = symbol.upper()
    if symbol_upper.endswith('.TW'):
        # 台灣證券交易所
        return mcal.get_calendar('XTAI')
    # 未來可在此處擴充其他市場，例如：
    # elif symbol_upper.endswith('.HK'):
    #     return mcal.get_calendar('XHKG') # 香港交易所
    # elif symbol_upper.endswith('.L'):
    #     return mcal.get_calendar('LSE') # 倫敦證券交易所
    else:
        # 對於無明確後綴的代碼（通常為美股）或指數，預設使用 NYSE
...
def verify_and_repair_data(symbol: str, data: pd.DataFrame, start_date: str, end_date: str) -> pd.DataFrame:
    ...
    此版本能動態選擇市場日曆，正確處理包括台股、美股及外匯在內的多市場資產。
    ...
    # 1. 【邏輯升級】根據資產類別，選擇合適的日曆生成策略
    try:
        if "=" in symbol:
            # 對於外匯 (FX)，使用通用商業日曆更為合適
            print(f"[{symbol}] 偵測為外匯，使用通用商業日曆進行驗證。")
            expected_dates = pd.bdate_range(start=start_date, end=end_date)
        else:
            # 對於股票或指數，使用市場感知的交易日曆
            calendar = get_market_calendar(symbol)
            print(f"[{symbol}] 使用 {calendar.name} 交易日曆進行驗證。")
            expected_dates = calendar.valid_days(start_date=start_date, end_date=end_date).tz_localize(None)

    except Exception as e:
        print(f"警告: 為 {symbol} 產生預期日曆 ({start_date} to {end_date}) 時出錯: {e}。跳過驗證。")
        return data
    ...
    # 3. 透過集合運算，高效計算出缺失的日期
    missing_dates = pd.to_datetime(expected_dates).difference(existing_dates)
    ...
    if not missing_dates.empty:
        ...
        for i, missing_date in enumerate(missing_dates):
            ...
            try:
                ...
                repair_df = repair_func()

                if repair_df is not None and not repair_df.empty:
                    repaired_data_list.append(repair_df)
                else:
                    print(f"  -> 資訊: {symbol} 在 {start_of_day} 確認無交易數據。")

                time.sleep(0.5)

            except Exception as e:
                if "YFPricesMissingError" in str(type(e)):
                     print(f"  -> 資訊: {symbol} 在 {start_of_day} 確認無交易數據 (YFPricesMissingError)。")
                else:
                     print(f"  -> 警告: 修復 {start_of_day} 時發生未預期錯誤: {e}")
...
def d1_batch(statements):
    ...
    def batch_func():
        response = requests.post(f"{D1_WORKER_URL}/batch", json={"statements": statements}, headers=headers, timeout=120)
        response.raise_for_status()
        return True
...
# =========================================================================================
def fetch_and_overwrite_market_data(targets, benchmark_symbols, global_earliest_tx_date, batch_size=5):
    if not targets:
        print("沒有需要刷新的標的。")
        return False
    ...
    for i, batch in enumerate(symbol_batches):
        ...
        for symbol in symbols_to_fetch_in_batch:
            ...
            if symbol_data_filtered.empty:
                print(f"資訊: {symbol} 在其指定的日期範圍內沒有初步數據，跳過驗證步驟。")
                continue
...
if __name__ == "__main__":
    print(f"--- 開始執行週末市場數據完整校驗腳本 (v3.7 - Multi-Market Aware Resilient) --- {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    try:
        import pandas_market_calendars as mcal
    except ImportError:
        print("FATAL: 缺少 'pandas_market_calendars' 函式庫。請執行 'pip install pandas_market_calendars' 進行安裝。")
        exit(1)
