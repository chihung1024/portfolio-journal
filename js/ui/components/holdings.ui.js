// =========================================================================================
// == 持股表格 UI 模組 (holdings.ui.js) - v3.0 (Responsive Refactor)
// =========================================================================================
            ...
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gray-50">
                    <tr>
                        ...
                        <th class="px-6 py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider">股數</th>
                        <th class="px-6 py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider">現價 / 成本</th>
                        <th class="px-6 py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider cursor-pointer hover:bg-gray-100" data-sort-key="marketValueTWD">市值(TWD) ${getSortArrow('marketValueTWD')}</th>
                        <th class="px-6 py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider cursor-pointer hover:bg-gray-100" data-sort-key="daily_pl_twd">當日損益 ${getSortArrow('daily_pl_twd')}</th>
                        <th class="px-6 py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider cursor-pointer hover:bg-gray-100" data-sort-key="unrealizedPLTWD">未實現損益 ${getSortArrow('unrealizedPLTWD')}</th>
                        <th class="px-6 py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider cursor-pointer hover:bg-gray-100" data-sort-key="portfolioPercentage">持股佔比 ${getSortArrow('portfolioPercentage')}</th>
                    </tr>
                </thead>
                <tbody class="bg-white divide-y divide-gray-200">
                    ${holdingsArray.map(h => { 
                        ...
                        return `
                            <tr class="hover:bg-gray-50 cursor-pointer holding-row" data-symbol="${h.symbol}" ${isShort ? 'style="background-color: #f0f9ff;"' : ''}>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-semibold text-right ${isShort ? 'text-sky-700' : ''}">${formatNumber(h.quantity, decimals)}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-right">
                                    <div class="text-sm font-semibold text-gray-900 rounded px-1 inline-block">${formatNumber(h.currentPriceOriginal, 2)}</div>
                                    <div class="text-xs text-gray-500">${formatNumber(h.avgCostOriginal, 2)} ${h.currency}</div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-right">${formatNumber(h.marketValueTWD, 0)}</td>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-right">${h.portfolioPercentage.toFixed(2)}%</td>
                            </tr>`; 
                    }).join('')}
                </tbody>
            </table>
            ...
            <div class="bg-white rounded-lg shadow overflow-hidden ${isShort ? 'ring-2 ring-sky-300' : ''}">
                <div class="px-4 py-3 flex justify-between items-center cursor-pointer list-view-item" data-symbol="${h.symbol}">
                    ...
                    <div class="text-right">
                        <p class="font-medium text-base text-gray-900">${formatNumber(h.marketValueTWD, 0)}</p>
