// =========================================================================================
// == 檔案：js/ui/components/groups.ui.js (新增檔案)
    ...
    if (allSymbols.length > 0) {
        symbolsContainer.innerHTML = allSymbols.map(symbol => `
            <label class="flex items-center space-x-3 p-2 rounded-md hover:bg-gray-100">
                <input type="checkbox" name="group_symbols" value="${symbol}" class="h-4 w-4 text-indigo-600 border-gray-300 rounded" ${groupSymbols.has(symbol) ? 'checked' : ''}>
