...
function calculateFinalHoldings(pf, market, allEvts) {
  ...
  for (const sym in pf) {
    ...
    if (Math.abs(qty_end_of_day) > 1e-9) {
      ...
      let latestDateStr = new Date().toISOString().split('T');
      let beforeDateStr = new Date().toISOString().split('T');

      if (availableDates.length > 0) {
        latestDateStr = availableDates;
        latestPrice = symbolPrices[latestDateStr];
        if (availableDates.length > 1) {
          beforeDateStr = availableDates[15];
      ...
      // 今日該標的的配息現金流（新增，與 calculateDailyPL 一致，視為負現金流）
      const todaysDividends = allEvts.filter(e =>
        (e.eventType === 'confirmed_dividend' || e.eventType === 'implicit_dividend') &&
        e.symbol.toUpperCase() === sym &&
        toDate(e.date).getTime() === today.getTime()
      );
      todaysDividends.forEach(e => {
        if (e.eventType === 'confirmed_dividend') {
          const fx = findFxRate(market, e.currency, toDate(e.date));
          const dividendAmountTWD = e.amount * (e.currency === 'TWD' ? 1 : fx);
          dailyCashFlowTWD -= dividendAmountTWD; // 視為負現金流
        } else { // implicit_dividend
          const stateOnExDate = getPortfolioStateOnDate(allEvts, toDate(e.ex_date), market);
          ...
          if (shares > 0) {
            ...
            const dividendAmountTWD = postTaxAmount * shares * (currency === "TWD" ? 1 : fx);
            dailyCashFlowTWD -= dividendAmountTWD; // 視為負現金流
          }
        }
      });
      ...
      const denominator = beginningMarketValueTWD + dailyCashFlowTWD; // 日級 MDM 近似分母
    ...
    switch (e.eventType) {
      case "transaction": {
        ...
        if (e.type === "buy") {
          ...
          while (buyQty > 0 && pf[sym].lots.length > 0 && pf[sym].lots.quantity < 0) {
            const shortLot = pf[sym].lots;
          ...
          pf[sym].lots.sort((a, b) => a.date - b.date);
          while (sellQty > 0 && pf[sym].lots.length > 0 && pf[sym].lots.quantity > 0) {
            const longLot = pf[sym].lots;
