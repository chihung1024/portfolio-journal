// =========================================================================================
// == 檔案：functions/performRecalculation.js (v_final_robust + NetProfit Baseline Carryover)
...
async function maintainSnapshots(uid, newFullHistory, evts, market, createSnapshot = false, groupId = 'all') {
  ...
  const snapshotOps = [];
  const existingSnapshotsResult = await d1Client.query(
    'SELECT snapshot_date FROM portfolio_snapshots WHERE uid = ? AND group_id = ?',
    [uid, groupId]
  );
  const existingSnapshotDates = new Set(existingSnapshotsResult.map(r => r.snapshot_date.split('T')));
  ...
  if (latestDateStr && (createSnapshot || !existingSnapshotDates.has(latestDateStr))) {
    ...
    const totalCost = Object.values(finalState).reduce(
      (s, stk) => s + stk.lots.reduce((ls, l) => ls + l.quantity * l.pricePerShareTWD, 0),
      0
    );
    snapshotOps.push({
...
async function calculateAndCachePendingDividends(uid, txs, userDividends) {
  ...
  const confirmedKeys = new Set(userDividends.map(d => `${d.symbol.toUpperCase()}_${d.ex_dividend_date.split('T')}`));
  const holdings = {};
  let txIndex = 0;
  ...
  allMarketDividends.forEach(histDiv => {
    ...
    const exDateStr = histDiv.date.split('T');
    ...
    if (quantity > 0.00001) {
      const currency =
        txs.find(t => t.symbol.toUpperCase() === divSymbol)?.currency || (isTwStock(divSymbol) ? 'TWD' : 'USD');
...
async function performRecalculation(uid, modifiedTxDate = null, createSnapshot = false) {
  console.log(`--- [${uid}] 儲存式重算程序開始 (v_snapshot_integrated + NetProfitBaseline) ---`);
  try {
    ...
    // 抓取原始資料與既有摘要（含 netProfitHistory）
    ...
    let oldHistory = {};
    let previousNetProfitHistory = null;
    ...
    if (createSnapshot === false) {
      ...
      baseSnapshot = latestValidSnapshotResult;

      if (baseSnapshot) {
        console.log(`[${uid}] 找到基準快照: ${baseSnapshot.snapshot_date}，將執行增量計算。`);
        await d1Client.query(
          'DELETE FROM portfolio_snapshots WHERE uid = ? AND group_id = ? AND snapshot_date > ?',
          [uid, ALL_GROUP_ID, baseSnapshot.snapshot_date]
        );

        if (summaryResult) {
          if (summaryResult.history) {
            oldHistory = JSON.parse(summaryResult.history);
            Object.keys(oldHistory).forEach(date => {
              if (toDate(date) > baseSnapshot.snapshot_date) delete oldHistory[date];
            });
          }
          if (summaryResult.netProfitHistory) {
            const parsed = JSON.parse(summaryResult.netProfitHistory);
            // 僅保留至快照日（引擎會以快照日前一日為基線鍵）
            Object.keys(parsed).forEach(date => {
              if (toDate(date) > baseSnapshot.snapshot_date) delete parsed[date];
            });
            previousNetProfitHistory = parsed;
          }
        }
      } else {
    ...
    // 呼叫統一引擎（新增 previousNetProfitHistory 參數）
