...
// == 【全新】僅供群組使用的即時計算函式
// ==========================================================
async function performCalculationBySymbols(uid, symbols = []) {
    console.log(`--- [${uid}] 開始針對符號列表的即時計算 ---`);
    if (!symbols || symbols.length === 0) {
        return {
            summaryData: { totalRealizedPL: 0, xirr: 0, overallReturnRate: 0, benchmarkSymbol: 'SPY' },
            holdingsToUpdate: {},
            newFullHistory: {},
            twrHistory: {},
            benchmarkHistory: {},
    ...
    try {
        ...
        const [txs, splits, controlsData, userDividends] = await Promise.all([
            d1Client.query(`SELECT * FROM transactions WHERE uid = ? AND symbol IN (${symbolPlaceholders}) ORDER BY date ASC`, [uid, ...symbols]),
            d1Client.query(`SELECT * FROM splits WHERE uid = ? AND symbol IN (${symbolPlaceholders})`, [uid, ...symbols]),
            d1Client.query('SELECT value FROM controls WHERE uid = ? AND key = ?', [uid, 'benchmarkSymbol']),
            d1Client.query(`SELECT * FROM user_dividends WHERE uid = ? AND symbol IN (${symbolPlaceholders})`, [uid, ...symbols]),
        ]);
        ...
        // --- 以下計算邏輯與 performRecalculation 基本相同，但不包含快照和寫入操作 ---

        const benchmarkSymbol = controlsData.length > 0 ? controlsData[0].value : 'SPY';
        const currencies = [...new Set(txs.map(t => t.currency))].filter(c => c !== "TWD");
        const fxSymbols = currencies.map(c => currencyToFx[c]).filter(Boolean);
        const allRequiredSymbols = [...new Set([...symbols, ...fxSymbols, benchmarkSymbol.toUpperCase()])].filter(Boolean);
        ...
        if (!firstBuyDate) {
            return { summaryData: {}, holdingsToUpdate: {}, newFullHistory: {} }; // Return empty if no valid events
        }
        
        const history = {};
        let curDate = new Date(firstBuyDate);
        const today = new Date();
        today.setUTCHours(0,0,0,0);
        ...
        const dailyCashflows = calculateDailyCashflows(evts, market);
        const { twrHistory, benchmarkHistory } = calculateTwrHistory(history, evts, market, benchmarkSymbol, firstBuyDate, dailyCashflows);
        const portfolioResult = calculateCoreMetrics(evts, market);
        ...
        for (const dateStr of sortedHistoryDates) {
            cumulativeCashflow += (dailyCashflows[dateStr] || 0);
            const marketValue = history[dateStr] || 0;
            netProfitHistory[dateStr] = marketValue - cumulativeCashflow;
        }
        ...
        console.log(`--- [${uid}] 即時計算完成 ---`);

        // 【關鍵】直接回傳計算結果，不寫入資料庫
        return {
            summaryData,
            holdingsToUpdate,
            newFullHistory: history,
            twrHistory,
            benchmarkHistory,
            netProfitHistory
        };
        ...
        console.error(`[${uid}] 即時計算期間發生嚴重錯誤：`, e);
        throw e;
    }
}

module.exports = { performRecalculation, performCalculationBySymbols };
