...
// 【移除】getPortfolioStateOnDate, dailyValue, prepareEvents 三個函式
...
async function performRecalculation(uid, modifiedTxDate = null, createSnapshot = false) {
    console.log(`--- [${uid}] 重新計算程序開始 (v4.0.3 - Refactoring) ---`);
    try {
        ...
        await Promise.all(Object.keys(market).map(symbol => dataProvider.ensureDataCoverage(symbol, txs[0].date.split('T')[0])));

        // 【修改】呼叫 state.calculator 中的函式
