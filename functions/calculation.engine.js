...
function calculateTwrHistory(dailyPortfolioValues, evts, market, benchmarkSymbol, startDate, dailyCashflows, log = console.log) { const dates = Object.keys(dailyPortfolioValues).sort(); if (!startDate || dates.length === 0) return { twrHistory: {}, benchmarkHistory: {} }; const upperBenchmarkSymbol = benchmarkSymbol.toUpperCase(); const benchmarkPrices = market[upperBenchmarkSymbol]?.prices || {}; const benchmarkCurrency = isTwStock(upperBenchmarkSymbol) ? "TWD" : "USD"; const startFxRate = findFxRate(market, benchmarkCurrency, startDate); const benchmarkStartPriceOriginal = findNearest(benchmarkPrices, startDate); if (!benchmarkStartPriceOriginal) { log(`TWR_CALC_FAIL: Cannot find start price for benchmark ${upperBenchmarkSymbol}.`); return { twrHistory: {}, benchmarkHistory: {} }; } const benchmarkStartPriceTWD = benchmarkStartPriceOriginal * startFxRate; 
    // 【修正】直接使用 dailyCashflows，因為它的正負號定義已經符合 TWR 公式需求
    // TWR 公式中，現金流入(buy)為正，現金流出(sell/dividend)為負
...
function calculateMetricsOnDate(evts, market, targetDateStr) {
    const pf = {};
    const targetDate = toDate(targetDateStr);

    const relevantEvents = evts.filter(e => toDate(e.date) <= targetDate);
    ...
    for (const e of relevantEvents) {
        ...
        switch (e.eventType) {
            case "transaction": {
                ...
                if (e.type === "buy") {
                    pf[sym].lots.push({ quantity: e.quantity, pricePerShareTWD: costTWD / (e.quantity || 1), date: toDate(e.date) });
                } else {
            ...
            case "split": {
                pf[sym].lots.forEach(l => { l.quantity *= e.ratio; l.pricePerShareTWD /= e.ratio; });
                break;
            }
            ...
            case "implicit_dividend": {
                 const stateOnDate = getPortfolioStateOnDate(relevantEvents, toDate(e.ex_date), market);
    ...
    for (const sym in pf) {
        ...
        if (qty > 1e-9) {
            ...
            let priceOnDate = findNearest(market[sym]?.prices || {}, targetDate);
            const fx = findFxRate(market, h.currency, targetDate);
            const marketValueTWD = qty * (priceOnDate ?? 0) * (h.currency === "TWD" ? 1 : fx);
            const unrealizedPLTWD = marketValueTWD - totalCostTWD;
            const totalContribution = h.realizedPLTWD + unrealizedPLTWD;
...
async function getHistoricalHoldingsAnalysis(uid, targetDate) {
    const [txs, splits, userDividends] = await Promise.all([
        d1Client.query('SELECT * FROM transactions WHERE uid = ? AND date <= ? ORDER BY date ASC', [uid, targetDate]),
        d1Client.query('SELECT * FROM splits WHERE uid = ?', [uid]),
        d1Client.query('SELECT * FROM user_dividends WHERE uid = ?', [uid]),
    ]);

    const benchmarkSymbol = 'SPY'; // Benchmark symbol is not critical here, but needed for the function call
    const market = await getMarketDataFromDb(txs, benchmarkSymbol);
    const { evts } = prepareEvents(txs, splits, market, userDividends);

    const analysisData = calculateMetricsOnDate(evts, market, targetDate);
    return analysisData;
}

module.exports = { performRecalculation, getHistoricalHoldingsAnalysis };
