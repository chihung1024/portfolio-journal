...
- **⚡️ 即時報價更新**：提供手動更新按鈕，可立即抓取持股的最新市場報價，並在前端即時重新計算資產價值。
- **🔄 自動化數據同步**：
    - **自動刷新**：登入後，前端頁面會定時自動從後端同步最新數據。
...
| 元件 | 技術/服務 | 職責 |
| --- | --- | --- |
| **前端 (Frontend)** | `index.html` (HTML, JS, Tailwind CSS) | UI/UX、使用者認證、API 請求、畫面渲染、輕量計算 |
...
1.  **使用者認證與資料載入**：
    ...
    - 驗證通過後，後端從 D1 資料庫讀取該使用者的數據，執行計算，並將結果返回給前端。
...
3.  **每日自動化維護**：
    - **Cloud Scheduler** 等排程工具在每日指定時間，觸發執行 **Python 腳本 (`main.py`)**。
    - 腳本向 D1 查詢所有使用者正在關注的標的（持股與 Benchmark）。
    - 腳本透過 `yfinance` 向 **Yahoo Finance API** 請求這些標的所需範圍內的歷史價格與股息。
...
- **即時的操作回饋**：所有需要等待後端處理的操作（如儲存交易、更新報價）都會有讀取動畫，並在完成後跳出成功或失敗的提示訊息。
- **響應式設計**：基於 Tailwind CSS，確保在桌面和行動裝置上都有良好的瀏覽體驗。
- **可控的刷新機制**：除了定時自動刷新，使用者還可以透過「更新即時報價」按鈕，主動獲取截至當下的最新資產價值。
